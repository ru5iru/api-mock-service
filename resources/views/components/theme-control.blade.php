@props(['variant' => 'menu', 'name' => 'theme'])

@if ($variant === 'menu')
    <details class="theme-menu">
        <summary class="theme-trigger" data-theme-trigger aria-label="Theme: system. Choose theme.">
        ...
        <div class="theme-menu-panel" role="group" aria-label="Theme">
            @foreach (['system' => 'System', 'light' => 'Light', 'dark' => 'Dark'] as $value => $label)
                <label>
                    <input type="radio" name="{{ $name }}" value="{{ $value }}" data-theme-option>
                    <span>{{ $label }}</span>
                </label>
            @endforeach
        </div>
...
@else
    <fieldset class="theme-radio-group">
        <legend>Theme</legend>
        @foreach (['system' => 'System', 'light' => 'Light', 'dark' => 'Dark'] as $value => $label)
            <label>
                <input type="radio" name="{{ $name }}" value="{{ $value }}" data-theme-option>
